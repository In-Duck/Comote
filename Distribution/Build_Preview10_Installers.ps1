...
if ([string]::IsNullOrWhiteSpace($PackagePath)) {
    $PackagePath = Join-Path $root `
        "artifacts\Comote-1.6.0-preview.17-win-x64.zip"
}

if (-not (Test-Path -LiteralPath $PackagePath)) {
    throw "Preview 17 package not found: $PackagePath"
}
...
$installer = Join-Path $outputDir `
    "ComoteClient_Setup_1.6.0-preview.17_Offline.exe"
